function x = gmresir(A,b,precf,precw,precr,iter_max,savename,restrt)
...
while ~cged
    ...
    if precw == 2
        [d, err, its, ~] = gmres_dq( A, zeros(n,1), double(rd1), LL, U, restrt, gtol);
    else
        [d, err, its, ~] = gmres_sd( A, single(zeros(n,1)), single(rd1), LL, U, restrt, gtol);
    end
