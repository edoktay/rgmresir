% GCRODR   GCRO with Deflated Restarting with double/quadruple precision, 
...
function [x,resvec,r,nmv,relres] = gcrodr_dq(A,b,m,k,x0,tol,M1,M2,reuse_name)
...
r = b - A*x0;
r = double(mp(M2,34)\(mp(M1,34)\mp(r,34)));
...
if(isfield(U_persist,reuse_name))
   ...
   C = double(mp(M2,34)\(mp(M1,34)\(mp(A,34)*mp(U,34))));
 

   % Orthonormalize C and adjust U accordingly so that C = A*U
   [C,R] = qr(C,0);
   U = double(mp(U,34) / mp(R,34));
   ...
   [x,r,V,H,p,resvec_inner] = gmres1_dq(A,x,r,m,M1,M2,tol*bnorm);
...
while(resvec(nmv) / bnorm > tol)
   ...
   [V,H,B,p,resvec_inner] = gmres2_dq(A,r,m-k,M1,M2,C,tol*bnorm);
